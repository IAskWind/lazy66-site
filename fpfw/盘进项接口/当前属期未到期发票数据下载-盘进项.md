...
- 接口描述：`当前属期未到期发票数据下载`
- 接口名称：`lazy.opweb.pjx.dqssq_wdq_fpsjxz`
- 报文版本：1.0
- 接口说明：查询下载企业当前税款所属期，未到勾选日期的发票数据。如果税款所属期为当前月，不下载未到期数据。
...
|  5   |  kprqq   |  开票日期起  |  10  |  是  | 格式：YYYY-MM-DD<br/>开票日期起   最小为当前月的第一天  |
|  6   |  kprqz   |  开票日期止  |  10  |  是  | 格式：YYYY-MM-DD<br/>开票日期止    最大为当前月当天日期 |
|  7   |   fpdm   |   发票代码   |  12  |  否  |                                                         |
|  8   |   fphm   |   发票号码   |  8   |  否  |                                                         |
|  9   |  start   |   起始条数   |      |  否  | 默认值0                                                 |
|  10  | pageSize |   下载条数   |      |  否  | 默认值500                                               |
